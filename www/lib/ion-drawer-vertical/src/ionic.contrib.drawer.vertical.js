(function () {
    ...
    .controller('$ionDrawerVertical', function ($scope, $element, $attrs, $ionicGesture, $timeout, $q, $ionicHistory, $ionDrawerVerticalDelegate, $ionicScrollDelegate) {
        ...
        var $handle = $element.find('ion-drawer-vertical-handle');
         ...
         //DOM items height
        var headerHeight = document.querySelector('ion-header-bar').clientHeight;
        var handleHeight = $handle[0].clientHeight;
        var topHeaderMargin = headerHeight + handleHeight > 0 ? headerHeight + handleHeight : 0;
        ...
        var open = function () {
            ...
            if ((isClosed() ||  isDoneDragging()) && !isBusyAnimating()) {
                ...
                if (direction === DIRECTION_DOWN)
                    $wrapper[0].style[ionic.CSS.TRANSFORM] = 'translate3d(0,'+ -topHeaderMargin +'px, 0)';
                else
                    $wrapper[0].style[ionic.CSS.TRANSFORM] = 'translate3d(0,'+ topHeaderMargin +'px, 0)';
                
                $wrapper[0].style.zIndex = 9;
        ...
        var handleDrag = function (deltaY) {
            ...
            if (direction == DIRECTION_DOWN) {
                ...
                deltaY = limitNumberBetween(deltaY, margin - height, -topHeaderMargin);
            }
            if (direction == DIRECTION_UP) {
                if (prevState == STATE_CLOSE) {
                    deltaY += height - margin;
                }
                deltaY = limitNumberBetween(deltaY, topHeaderMargin, height - margin);
            }
